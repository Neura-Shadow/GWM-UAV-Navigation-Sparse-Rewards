...
try:
    ...
    try:
        ...
        raise ImportError(
            "找不到 AirSim Python 套件，請安裝其中之一: pip install cosysairsim 或 pip install airsim"
        ) from exc
...
class PlannerConfig:
    ...
    airsim_timeout_sec: int = 30
    connect_retries: int = 8
    connect_retry_interval: float = 1.5
    goal_x: float = 60.0
    goal_y: float = 20.0
    goal_z: float = -8.0
...
class AirSimNeuroPlanner:
    def __init__(self, cfg: PlannerConfig):
        ...
        self.client = airsim.MultirotorClient(
            ip=cfg.airsim_host,
            port=cfg.airsim_port,
            timeout_value=cfg.airsim_timeout_sec,
        )
    ...
    def connect_and_prepare(self) -> None:
        print(
            f"[INFO] 連線到 AirSim... host={self.cfg.airsim_host}, port={self.cfg.airsim_port}"
        )
        ...
        for attempt in range(1, self.cfg.connect_retries + 1):
            try:
                ...
                last_error = exc
                print(
                    f"[WARN] 連線失敗 ({attempt}/{self.cfg.connect_retries}): {exc}"
                )
                time.sleep(self.cfg.connect_retry_interval)

        if last_error is not None:
            raise RuntimeError(
                "無法連線到 AirSim RPC 伺服器。"
                f"\n- 目前設定: host={self.cfg.airsim_host}, port={self.cfg.airsim_port}"
                "\n- 請先啟動 AirSim 場景（Unreal/Blocks）再執行程式"
                "\n- 若模擬器在不同主機，請設定環境變數:"
                "\n  AIRSIM_HOST=<ip> AIRSIM_PORT=<port> python main.py"
            ) from last_error
