"""CLI for the guarded Phase 5-C ROS2 sensor sync smoke test."""
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Run a guarded ROS2 sensor synchronization smoke test."
    )
    ...
    parser.add_argument(
        "--fail-on-unavailable",
        action="store_true",
        help="Return nonzero if the env gates are set but ROS2 sensor sync modules are unavailable.",
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    """Run the guarded ROS2 sensor sync smoke test."""
...
def _summary_line(result: Dict[str, Any]) -> str:
    return (
        ...
        f"packets={result.get('packets_completed')}/{result.get('packets_requested')} "
...
def _exit_code_for_result(result: Dict[str, Any], fail_on_unavailable: bool) -> int:
    ...
    if (
        ...
        and result.get("availability", {}).get("ros2_sensor_sync_available") is False
    ):
        return 2
    return 0
